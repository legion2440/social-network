...
const latestMigrationVersion uint = 16
...
func guardMediaOnlyDownMigration(db *sql.DB) error {
	if db == nil {
	...
	if err := db.QueryRow(`
		SELECT
			(SELECT COUNT(*) FROM posts WHERE text = '' AND media_id IS NOT NULL)
			+
			(SELECT COUNT(*) FROM post_comments WHERE text = '' AND media_id IS NOT NULL)
	`).Scan(&mediaOnly); err != nil {
		return fmt.Errorf("check media-only content before down migration: %w", err)
	}
	if mediaOnly > 0 {
		return errors.New("cannot migrate down while media-only posts or comments exist")
	}
	return nil
}
