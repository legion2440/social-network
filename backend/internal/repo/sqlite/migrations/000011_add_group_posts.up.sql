CREATE TABLE autoincrement_000011_backup (
  name TEXT PRIMARY KEY,
  seq INTEGER NOT NULL
);

INSERT INTO autoincrement_000011_backup (name, seq)
SELECT name, seq
...
SET seq = max(
  seq,
  COALESCE((SELECT saved.seq FROM autoincrement_000011_backup saved WHERE saved.name = 'posts'), 0),
...
WHERE NOT EXISTS (SELECT 1 FROM sqlite_sequence WHERE name = 'posts')
  AND (
    EXISTS (SELECT 1 FROM autoincrement_000011_backup WHERE name = 'posts')
...
SET seq = max(
  seq,
  COALESCE((SELECT saved.seq FROM autoincrement_000011_backup saved WHERE saved.name = 'post_comments'), 0),
...
WHERE NOT EXISTS (SELECT 1 FROM sqlite_sequence WHERE name = 'post_comments')
  AND (
    EXISTS (SELECT 1 FROM autoincrement_000011_backup WHERE name = 'post_comments')
    OR EXISTS (SELECT 1 FROM post_comments)
  );

DROP TABLE autoincrement_000011_backup;
