CREATE TABLE autoincrement_000016_down_backup (
  name TEXT PRIMARY KEY,
  seq INTEGER NOT NULL
);

INSERT INTO autoincrement_000016_down_backup (name, seq)
SELECT name, seq
FROM sqlite_sequence
WHERE name IN ('posts', 'post_comments');

CREATE TABLE posts_000016_down (
...
INSERT INTO posts_000016_down (id, author_user_id, group_id, text, privacy, media_id, created_at)
SELECT id, author_user_id, group_id, text, privacy, media_id, created_at
FROM posts;

CREATE TABLE post_selected_users_000016_down_backup AS
SELECT post_id, user_id FROM post_selected_users;

CREATE TABLE post_comments_000016_down_backup AS
SELECT id, post_id, author_user_id, text, media_id, created_at FROM post_comments;

DROP TABLE post_selected_users;
DROP TABLE post_comments;
DROP TABLE posts;
ALTER TABLE posts_000016_down RENAME TO posts;
...
SELECT post_id, user_id FROM post_selected_users_000016_down_backup;

DROP TABLE post_selected_users_000016_down_backup;
...
FROM post_comments_000016_down_backup;

DROP TABLE post_comments_000016_down_backup;
...
SELECT name, seq FROM autoincrement_000016_down_backup;

DROP TABLE autoincrement_000016_down_backup;
